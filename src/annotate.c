...
char *mo_fetch_annotation_links (char *url, int on_top)
{
  ...
  /*
   * With group annotations, we may have to merge the group and private
   * annotations into a single string.
   */
  ...
  if (on_top)
    ...
    {
      strcpy (msg, "______________________________________\n");
      strcat (msg, pans);
    }
  free(pans);
  return msg;
 ...
 */
      if (win->pubpri == mo_annotation_public ||
          win->pubpri == mo_annotation_workgroup)
        {
		   ...
		   */
		  mo_new_grpan(win->current_node->url, title, author, txt);
		   ...
		   */
		  mo_modify_grpan(win->current_node->url, title, author, txt);
    ...
    case 2:
      mo_open_another_window
        (win,
         "http://www.ncsa.uiuc.edu/SDG/Software/Mosaic/Docs/help-on-annotate-win.html",
         NULL, NULL);
      break;
    }
