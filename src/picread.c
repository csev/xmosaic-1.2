...
/* Colorado District Hack by S. Townsend 6-12-1993 */

unsigned char *ReadXpm3Pixmap(fp, datafile, w, h, colrs)
FILE *fp;
char *datafile;
int *w, *h;
XColor *colrs;
{
        int Colors=0, CharsPP=0;
        ...
        *h=0;
        for( ; ;)
        {
           ...
           {
/*              printf("An error occured in getting image size, colors, and Chars. per pixel!\n"); */
              return(NULL);
           }
           if(sscanf(line,"\"%d %d %d %d \",",w,h,&Colors,&CharsPP)==4)
              break;
        }
...
XColor *colrs;
{
	...
	if (fp != NULL)
	{
		...
		bit_data = ReadXpm3Pixmap(fp, datafile, w, h, colrs);
		if (bit_data != NULL)
		{
			if (fp != stdin) fclose(fp);
			return(bit_data);
